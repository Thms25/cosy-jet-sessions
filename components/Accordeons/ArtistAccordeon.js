...
export default function ArtistAccordeon({ videos }) {
  const [open, setOpen] = useState(items[0].id);
  return (
    <div className="flex flex-col lg:flex-row h-fit lg:h-[480px] w-full max-w-4xl mx-auto shadow overflow-hidden">
      {videos.map((item) => {
        return (
          <Panel
            key={item.id}
...
const Panel = ({ open, setOpen, id, title, imgSrc, description }) => {
  ...
  return (
    <>
      <button
        className="bg-cjsWhite hover:bg-cjsPink transition-colors p-3 border-r-[1px] border-b-[1px] border-cjsWhite flex flex-row-reverse lg:flex-col justify-end items-center gap-4 relative group"
        onClick={() => setOpen(id)}
      >
        <span
          style={{
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="w-full h-full overflow-hidden relative bg-cjsBrown flex items-end"
          >
            <motion.div
              ...
              animate="open"
              exit="closed"
              className="px-4 py-2 bg-black/40 backdrop-blur-sm text-cjsWhite"
            >
              <p>{description}</p>
            </motion.div>
          </motion.div>
